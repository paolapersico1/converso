...
class SpeechCorrector:
    ...
    def __init__(self) -> None:
        ...
        unigram_df = pd.read_csv(
            "it.word.top_unigrams_prob.csv",
            index_col=["ngram"],
            usecols=["ngram", "frequency", "prob"],
        )
        bigram_df = pd.read_csv(
            "it.word.top_bigrams_prob.csv",
    ...
    def raw_count(self, ngram):
        ...
        if isinstance(ngram, str):
            if ngram in self.custom_words:
                return self.unigram_df.frequency.max()
            df = self.unigram_df
        else:
            if any(
                " ".join(ngram) in custom_ngram for custom_ngram in self.custom_ngrams
            ):
    ...
    def conditional_prob(self, left, right):
        ...
        if (left, right) in self.cached_cond_prob:
            ...
            result = self.lambda1 * cond_prob + (1 - self.lambda1) * self.prob(left)
    ...
    def correct(self, sentence):
        ...
        while errors:
            ...
            original_ll = self.sentence_LL(sequence)
        ...
        _LOGGER.debug("Elapsed time: ", time.perf_counter() - start_time)
    ...
    def replace_with_best_candidate(self, sentence, candidates, stage, original_ll):
        ...
        if candidates:
            ...
            _LOGGER.debug("Best candidate: " + best_candidate)
            if (best_ll_bi - original_ll) > self.improv_threshold[stage]:
