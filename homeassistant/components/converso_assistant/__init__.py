"""The Converso Assistant integration."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Converso Assistant from a config entry."""

    hass.data.setdefault(DOMAIN, {})
    converso_agent = ConversoAgent(hass, entry)
    conversation.async_set_agent(hass, entry, converso_agent)
    await converso_agent.async_initialize(hass.data.get("conversation_config"))
...
class ConversoAgent(agent.AbstractConversationAgent):
    """Converso agent."""

    def __init__(self, hass: core.HomeAssistant, entry: ConfigEntry) -> None:
    ...
    def _get_or_load_intents(
        ...
        _LOGGER.debug("Loaded intents=%s", lang_intents)
    ...
    def _make_slot_lists(self) -> dict[str, SlotList]:
        ...
        # _LOGGER.error("Exposed entities: %s", self.hass.states.async_all())
    ...
    async def _match_triggers(self, sentence: str) -> agent.ConversationResult | None:
